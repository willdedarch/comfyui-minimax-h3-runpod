"""Runpod disposable MiniMax H3 I2V bootstrap. No persistent volume required."""
import hashlib
import json
import os
...
def main():
    ...
    for _, _, source, _, _, _ in MODELS:
        if Path(source).name not in all_text:
            raise RuntimeError("Official workflow does not reference " + source)
    # Keep the original graph intact; provide its stock input when available.
    for node in workflow.get("nodes", []):
        if node.get("type") == "LoadImage":
            ...
            try:
                ...
                print("Upload your first-frame image before generating.", flush=True)
    (root / "h3-model-manifest.json").write_text(json.dumps(MODELS, indent=2))
    print("H3: five model hashes verified. Starting official Runpod services.", flush=True)
